...
pub fn hanoi(num_discs: u32, src: Peg, aux: Peg, dst: Peg) -> Vec<Move> {
    ...
    // move all but the last disc from src to aux
    if num_discs > 1 {
        moves.extend_from_slice(&hanoi(num_discs - 1, src, dst, aux));
    }

    // move the last disc from src to dst
    moves.push((src, dst));

    // move all the discs from aux to src
    if num_discs > 1 {
        moves.extend_from_slice(&hanoi(num_discs - 1, aux, src, dst));
    }
